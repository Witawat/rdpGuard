...
| `poll_retry_min_seconds` | `15` | หลายเครื่อง/bot เดียว: รอสุ่มขั้นต่ำก่อนลอง poll ใหม่หลังเจอ HTTP 409 |
| `poll_retry_max_seconds` | `45` | หลายเครื่อง/bot เดียว: รอสุ่มขั้นสูงก่อนลอง poll ใหม่หลังเจอ HTTP 409 |
...
>
> การรับคำสั่งใช้ **staggered polling**: เครื่องที่เจอ `HTTP 409 Conflict` (อีกเครื่องถือสิทธิ์รับคำสั่งอยู่) จะรอสุ่ม `poll_retry_min_seconds`–`poll_retry_max_seconds` วินาที แล้วลองใหม่ — ทุกเครื่องผลัดกันเป็นเจ้าของรับคำสั่ง ไม่มีเครื่องไหนถูกกันออกถาวร; ยิ่งช่วงสั้น ยิ่งสลับเจ้าของบ่อย (ได้คำสั่งไว) แต่ส่ง request มากขึ้น
