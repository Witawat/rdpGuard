...
- **แก้ Telegram Command ขึ้น `HTTP Error 409: Conflict`** เมื่อใช้ bot เดียวกับหลายเครื่อง — เพิ่ม staggered polling: แต่ละเครื่องเช็คสิทธิ์รับคำสั่งด้วย timeout สั้น; ถ้าอีกเครื่องถือสิทธิ์อยู่ (409) จะรอสุ่ม 15–45 วิ แล้วลองใหม่ ทำให้ทุกเครื่องผลัดกันเป็นเจ้าของรับคำสั่ง (ไม่มีเครื่องไหนถูกกันออกถาวร) — ตั้งช่วงเวลารอได้ใน ตั้งค่า → แจ้งเตือน (`poll_retry_min_seconds` / `poll_retry_max_seconds`)
